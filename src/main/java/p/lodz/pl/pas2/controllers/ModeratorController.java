...
public class ModeratorController {
    ...
    @Autowired
    public ModeratorController(UserService userService, PasswordEncoder passwordEncoder) {
    ...
    public ResponseEntity<User> addUser(@Valid @RequestBody ModeratorRequest user) {
        return ResponseEntity.status(HttpStatus.CREATED).body(userService.addUser(new Moderator(user.getUsername(), user.isActive(), passwordEncoder.encode(user.getPassword()))));
    }

    @PutMapping("/{id}")
    public ResponseEntity<User> updateUser(@PathVariable UUID id, @Valid @RequestBody ModeratorRequest user) {
        return ResponseEntity.status(HttpStatus.OK).body(userService.updateUser(id, new Moderator(user.getUsername(), user.isActive(), passwordEncoder.encode(user.getPassword()))));
    }
